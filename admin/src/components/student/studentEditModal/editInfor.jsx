...
export default function EditInfor({
  ...
  const getAllClass = async () => {
    try {
      ...
      setClasses(response.data.filter((classItem) => classItem.code !== student.class_code.code));
    } catch (err) {
      console.log(err);
    }
  ...
  const schema = yup.object().shape({
    ...
    class_role: yup.string().required("vui lòng chọn lớp."),
  });
  const {
    register,
    handleSubmit,
        ...
        <div className={`h-full mt-[20px] ${haveCloseModal?"overflow-y-scroll grow":""}`}>
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
            ...
            <div className={`${styles.item}`}>
              ...
              <div className="flex flex-col w-2/3 ">
                <select
                  defaultValue={student.class_code.code}
                  {...register("class_code")}
                ...
                >
                    <option value={student.class_code.code}>{student.class_code.code}</option>
                {classes.map((item) => (
            ...
            <div className={`${styles.item}`}>
              ...
              <div className="flex flex-col w-2/3">
                <select
                  defaultValue={student.role_id}
                  className="block w-full px-3 py-1 text-sm h-12 rounded-md bg-gray-100 focus:bg-gray-50 border-[1px] focus:bg-transparent focus:outline-none"
                  {...register("role_id")}
                >
                  <option value={student.account_id.role_id.id}>{student.account_id.role_id.name}</option>
                </select>
              </div>
            </div>
